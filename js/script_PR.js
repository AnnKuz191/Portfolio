...
function initGallery() {
    allGalleryImages = Array.from(document.querySelectorAll('.gallery-img-container img, .simple-img-container img'));
    ...
    allGalleryImages.forEach((img, index) => {
        img.addEventListener('click', (e) => {
            ...
            createLightbox(img.dataset.full || img.src, img.alt, index);
        });
        
        img.style.cursor = 'zoom-in';
    });
    
    document.querySelectorAll('.zoom-icon, .simple-zoom-icon').forEach((icon) => {
        icon.addEventListener('click', (e) => {
            e.stopPropagation();
            e.preventDefault();
            
            let img;
            if (icon.classList.contains('simple-zoom-icon')) {
                img = icon.closest('.simple-img-container').querySelector('img');
            } else {
                img = icon.closest('.gallery-img-container').querySelector('img');
            }
            
            if (img) {
                currentGalleryIndex = allGalleryImages.indexOf(img);
                createLightbox(img.dataset.full || img.src, img.alt, currentGalleryIndex);
            }
        });
    });
}
